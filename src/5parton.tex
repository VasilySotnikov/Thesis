...
\todo{reference Wjj benchmark}

\section{Definition of Amplitudes}
...
%
\begin{align}\begin{split} \label{eqn:Iop}
		\mathbf{I}^{(2)}_{[n]}(\eps)=&
...
\todo{possibly here section ``Implementaiton Details''}
...
\eqns{eq:partials}{eq:nfdecomposition}. All results are presented
in the HV scheme.
%
\begin{table}[h]
  \begin{adjustbox}{width=1\textwidth}
...
We set the regularization scale~$\mu$ to 1 and the normalization
of the results is fixed by the expansion in 
\eqns{eq:partials}{eq:nfdecomposition}. All results have been computed in the 
...
\todo{Strucutre here is not yet finalized}
...
A Leǐnartas decomposition \eqref{eq:LeinartasDecomposition}
