...
Having the on-shell loop-momenta components available,
...
momenta into a space with finite integer $D$.
Taking advantage of this fact, we can embed the states of particles in a space of finite integer dimension $D_s\geq D$ as well.
From now on we will keep $D_s$ and $D$ separate,
and only after all master coefficients are determined will we set $D_s\to D\to 4-2\varepsilon$.
In \cref{chap:dshel} we investigate closely how to deal with the $D_s$-dependence.
...
momenta $\{p_1,\ldots{},p_n\}$ into an index of type $\bar{\alpha}$.
...
\begin{subequations}
  \begin{align}
    ...
    \qty(\slashed{\ell}  \pm m)^{\alpha\beta} &~\xleftrightarrow{\ell^2\to m^2}~ \sum_{j=1}^{2^{\frac{D_s}{2}-1}} u^\alpha_j(\ell){\overline{u}_j^\beta}(\ell),
  \end{align}
\end{subequations}
...
Only at the of the path do we contract the final current with a state from a polarization sum.
Note that beyond two loops it is not always possible to span the whole cut with just one continuous path.

\begin{figure}[ht]
  \centering
  \subfloat[\label{sf:topo_cut:path}]{
    \includegraphics[width=0.3\textwidth]{topo_cut}
  }
  \hfill
  \subfloat[\label{sf:topo_cut:unrolled}]{
  ...
  \caption{A generic cut of a two-loop amplitude as a sequence of ``closed'' off-shell currents, shown as blobs.
    The red line in \cref{sf:topo_cut:path} demonstrates a path, which is ``unrolled'' in \cref{sf:topo_cut:unrolled}.
    Only the two cut-propagator numerators at the beginning and the end of the path are explicitly expanded in terms of polarization states.
  }
  \label{fig:topo_cut}
\end{figure}
...
We now have discussed both sides of \cref{eq:unitarity_ansatz},
and hereby everything required to obtain the coefficients of master integrals $c_{\Gamma\gamma_\Gamma,i}(\vb*{x},D,D_s)$. \todo{equation solving?} 
The coefficients are functions of external kinematics $\vb*{x}$, and the dimensional regularization parameters $D$ and $D_s$.
In the spirit of our goal to have a fully numerically-viable framework, 
...
and any (large enough to embed the loop momenta components in) \emph{integer} value of $D_s$.
The internal self-consistency of dimensional regularization guarantees
that these evaluations reproduce correctly the corresponding limits from the formally infinite-dimensional spaces.
...
and reconstruct the rational dependence on $D$ and polynomial dependence on $D_s$ from numerical samples for each coefficient.
This idea is known by the name 
\emph{dimensional reconstruction} \cite{Giele:2008ve,Ellis:2008ir,Boughezal:2011br,Abreu:2017xsl,Abreu:2017hqn}.

We conclude with some technical remarks which
have important implications for numerical efficiency of the approach.

The former comes solely from the right hand side of \cref{eq:unitarity_ansatz}, the ansatz,
 and the latter --- exclusively from the left hand side.

Although we do need to determine the surface coefficients for subtractions,
we do not need to reconstruct this dependence. This dependence is expected to be more complicated, because unphysical.
...
Another: 
even when we aim to evaluate external kinematics with floating point,
we can reconstruct the dependence on $D$ from finite field evaluations.

Another:
evaluating multiple $D_s$ values might be rather costly,
we see how to better do it \cref{chap:dshel} 
