...
define an algebraic variety in a $(6\cdot L)$-dimensional space.
...
We begin by considering all strands (see \cref{fig:topo_kinematics}) of the topology $\Gamma$ independently.
First we separate four-dimensional part and $(D-4)$-dimensional parts as
\begin{equation}
  \ell^l = \ell_{[4]}^l + \sum_i \tilde{n}_i~\mu^l_{i}, \qquad \sp(\ell^l_{[4]},\tilde{n}) = 0,
\end{equation}
...
\begin{enumerate}
  \item momenta flowing out of strand $l$ (added to set $B^l_p$),
  \item any other external momenta until $\max(4,N-1)$ of them is collected (added to set $B^l_t$)
\end{enumerate}
Then for $\ell^l_{[4]}$ we get
\begin{subequations}
  \begin{align}
    \ell^l_{[4]} &= \sum_{j\in B^l_p}\, r^l_j(\vb*{\rho}) v^l_j ~+~ \sum_{j\in B^l_t} \alpha^l_j\,v^l_j ~+~\sum_{j\in B_\tau} \alpha^\tau_j \frac{\tau_j}{(\tau_j)^2}, \\
    r_j^l(\vb*{\rho}) &=  \sp(\ell^l, p_j), \quad j\in B^l_p\\
...
reference appendix for $\mu_{ij}$


Related to ISPs we introduced before.
