...
First one extracts the list of all relevant topologies and numerator insertions from \cref{eq:general_amplitude},
identifies master integrals and computes them. 
Then one has to reduce all terms in \cref{eq:general_amplitude} to the form of \cref{eq:amplitude_integrated}.
All process- and model-dependent information is now contained in the master-integral coefficients $c_{\Gamma,i}$, so
the first part of the computation can be recycled for all kinematically-equivalent amplitudes.
...
then explain how unitarity methods allow to address some of it's difficulties in \cref{chap:numunitarity}

%The numerator functions $\mathcal{N}_{\Gamma,\gamma_\Gamma}$ are multivariate polynomials of loop momenta,
%which generate a large number of different monomials
...
In the HV scheme $D_{\text{ext}}=4$ and we the number of corresponding scalar products is
\begin{equation}
  \label{eq:sp_counts}
...
$\mathcal{K}[\vb*{sp}]$ with coefficients $c_{\va{i}}(\vb*{x},D)$ being rational functions of external kinematics $\vb*{x}$ and $D$:
\begin{multline}
...
An import observation which becomes evident from our choice of variables is that all topologies with $N_p> 4 L +\frac{1}{2}L(L+1)$, no matter the number of external particles, are reducible.
...
\begin{equation}
  \sum_{\Gamma \in \Delta} \sum_{\gamma} \sum_{\va{i}} ~ c_{\Gamma\gamma, \va{i}} (\vb*{x},D) ~ \qty(\int \prod_{i=1}^{L} \tilde{\dd{\ell_i}}) ~  
  \frac{m^{\va{i}}_{\Gamma\gamma}(\vb*{\alpha})%(\ell_1,\ldots,\ell_L,p_1,\ldots,p_{N-1})
    }{\prod_{j\in P_\Gamma} \rho_j^{\gamma_{j}}}.
  \label{eq:rsp_reduced_amplitude}
\end{equation}
The integrals in \cref{eq:rsp_reduced_amplitude} are process-independent and have the form of \cref{eq:amplitude_integrated}.
But we are still missing a large number of identities due to the vanishing of the total derivative in dimensional regularization.
...
\begin{equation}
  ...
  \frac{ v_k^\mu(\vb*{x},\vb*{\rho},\vb*{\alpha},D) \, m^{\va{i}}(\vb*{\alpha})
    }{\prod_{j} \rho_j^{\gamma_{j}}}
    ) = 0,
  \label{eq:ibps}
\end{equation}
here the components of \emph{IBP vectors} $v_k^\mu(\vb*{x},\vb*{\rho},\vb*{\alpha},D)$ are arbitrary functions
of all variables. The monomials $m^{\va{i}}(\vb*{\alpha})$ can be absorbed in the definition of the IBP vectors, but
it's useful to keep them separate.

\todo{abstract number of ISPs}

The relations generated from \cref{eq:ibps} suggest to think of all integrals as a vector space of lists
$I(\gamma_1,\ldots,\gamma_{N_p}; \alpha_1, \ldots, \alpha_{N_\text{ISP}})$


Families of integrals closed under IBP identities.

Systematic approach formulated by Laporta \cite{Laporta:2001dd}
...
\textsc{Kira} \cite{Maierhoefer:2017hyi,Maierhofer:2018gpa}
...
here very brief review of common methods to compute master integrals
