...
In this chapter, we give a brief overview of the theoretical framework
for the work presented in this thesis, as well as the basic notions that we
...
four known fundamental forces: the electromagnetic, weak, and strong force.
Together with General Relativity as a theory of gravity,
they describe the majority of observed phenomena in particle physics and astronomy.

The SM is a local quantum field theory formulated in a background Minkowski space-time.
The field theory's properties are described by the \emph{action}
\[
  S = \int \dd[4]{x}~\L(x),
\]
...
The Lagrangian of the SM can split into four sectors,
\begin{equation}
...
The interactions of particles are associated to local gauge symmetries of the action (or the Lagrangian).
In particular, the gauge group of the SM is
\begin{align}\label{eq:SMgauge}
  SU(3)_C\otimes SU(2)_W \otimes U(1)_Y,
\end{align}
where $SU(2)_W \otimes U(1)_Y$ is linked to the theory of \emph{electroweak} (EW)
interaction also known as the Glashow-Weinberg-Salam theory \cite{Glashow1961a,Weinberg1967a,Salam1968,Glashow1970},
and $SU(3)_C$ is linked to the strong force, or \emph{Quantum Chromodynamics} (QCD).
...
Here $f^{abc}$ and $g_s$ are the structure constants and the coupling of $SU(3)_C$ respectively.
Similarly $\varepsilon^{i,j,k}$ and $g$ are those of $ SU(2)_W$ gauge group. 
We will denote the coupling of the abelian group $U(1)_Y$ as $g^\prime$,
which does not show up here because of the absence of self-interactions.
...
\end{equation}
The cubic interaction terms arise from the \emph{covariant derivative} 
$$\slashed{D}\coloneqq D_{\mu}\gamma^\mu,$$
where $\gamma^{\mu}$ are Dirac gamma matrices, and $D_{\mu}$ is given by
\begin{equation}
  D_\mu = \partial_\mu
  +i\gs T^a G^a_\mu
  +i g I^i_W W^i_\mu
...
$SU(2)$ and $SU(3)$ respectively. The quantum number $Y_W$ is the weak hypercharge.
...
One way to generate particle masses, while still maintaining the gauge invariance of the action, is through 
\emph{spontaneous symmetry breaking} via the Higgs mechanism \cite{Higgs1964b,Higgs1964,Englert1964,Higgs1966}, in which
the symmetry group of the vacuum state is a subgroup of the full gauge group. 
...
\end{align}
We then expand the field $\Phi$ around its vacuum configuration, and reparametrize $\phi_0$ in terms
of real scalar fields $H$ and $\chi$,
\begin{align} \label{eq:Phiparam}
\Phi=\pmqty{\phi^+\\\frac{1}{\sqrt{2}}\left(v+H+i\chi\right)},
\end{align}
The four degrees of freedom of the Higgs field are distributed as follows:
...
In fact, for the Lagrangian of the  Higgs sector we then obtain
\begin{multline}
  \L_{\PH} 
...
Once the Higgs field is expanded around its vacuum expectation value the \cref{sm:eq:yuk}, evidently, contains non-diagonal bilinear terms.
To identify the mass eigenstates of the quarks fields, we 
rotate the left- and right-handed fields with unitary matrices $U^{f_{\rL}}$ and $U^{f_{\rR}}$ respectively,
...
This means that the naive integration with the ``classical'' action is ill-defined.
...
\end{equation}
which behave like fermions, in that they are represented by Grassmann (anti-commuting) fields in the path integral.
It can then be shown using the BRST symmetry that the correlation functions, computed with $\L_\text{Mod} $, do not depend on the choice of gauge fixing.
...
This will allow us to discard ghost contributions explicitly.
...
Instead, one can conveniently define the renormalized parameter at an arbitrary \emph{renormalization scale} $\mu$, given a renormalization prescription, such as \emph{modified minimal subtractions} (\MSbar).
...
This implies, that at high energies QCD behaves as a weakly-interacting theory. This phenomenon is known as \emph{asymptotic freedom}.
...
It is also one of the quantities which have a clean theoretical interpretation in the QFT framework.
...
\end{align}
where $p_i$ and $p_f$ are the sums of momenta of the initial and final states correspondingly, and we factored off explicitly the momentum-conserving $\delta$-function.
%The
...
which consist of combinations of propagators and vertices.
...
by the on-shell states, and dividing by suitable field renormalization factors.
...
\begin{enumerate}
  \item The degrees of freedom, associated to the in- and out- fields in perturbation theory, do not describe the propagating
    degrees of freedom of the full theory correctly. This is what happens in the QCD due to confinement.
...
This leads to a notion of the \emph{jet cross section}. Compared to parton cross sections, it is infrared safe by construction.
...
and this is an area of active research (see e.g.\ \cite{Somogyi:2006da,Somogyi:2006cz,GehrmannDeRidder:2005cm,Gaunt:2015pea,Czakon:2010td,Czakon:2011ve,Currie:2013vh,Catani:2007vq,Anastasiou:2003gr}).
...
If we choose $\ket{i} = \ket{f}$ we get 
...
Thus unitarity implies relations between different orders of perturbation theory.

Exploiting the fact the imaginary part of a propagator is given by
\begin{equation}
  \Im\frac{1}{p^2-m^2+i \varepsilon} \xrightarrow{\varepsilon \to 0} -\pi\delta(p^2-m^2),
\end{equation}
...
The exact singularity, as expected, does not appear in any physical process.
However, in perturbation theory we can analytically continue amplitudes to complex external momenta,
which we can always arrange to hit the poles explicitly while still satisfying on-shell conditions for external states.
...
In perturbation theory it is possible to generalize the application of cuts given by \cref{eq:cut} to
obtain multi-channel discontinuities \cite{Britto:2004nc}.
For an example see \cref{fig:quad_cut}.
...
This method uses the cut conditions \cref{eq:cut} to set some propagators on-shell and triangularize linear systems for determining the basis coefficients.
