...
of the scattering $\mathcal{T}$ defined in \cref{eq:tmatrix}.
...
\end{equation}
Here we used the definition in \cref{eq:matrix_element}. 
...
\todo{mention somewhere around here what is called on-shell} 
...
And the \emph{discontinuity} of an amplitude considered as a function of complex momenta,
\begin{equation}
...
can be obtained \cite{Cutkosky1960} by summing over all possible ways to \emph{cut} propagators in loops,
\begin{equation} \label{eq:cut}
  \frac{1}{p^2-m^2+i\varepsilon^2} ~\longrightarrow~ 2\pi i ~\delta(p^2-m^2) \Theta(p^{0})
\end{equation}

The optical theorem \cref{eq:optical_theorem} is closely related to
...
\end{mdframed}
If the model contains a one-particle state with mass $m_\xi$ (which can be zero),
and for some combination of momenta $p_\xi \equiv  \sum_{n=1}^{k} p_k$, $p_\xi^2 \to m_\xi^2$,
an amplitude of $n$ particles $A_n(1,\ldots{},n)$ factorizes as
\begin{multline} \label{eq:factorization}
  A_n(1,\ldots{},n) \longrightarrow \\
    A^\alpha_{k+1}(1,\ldots{},k;\xi) ~\dfrac{\sum_{i \in \text{states}} \xi^\alpha_i \bar\xi^\beta_i }{p_\xi^2-m_\xi^2}~ A^\beta_{n-k+1}(k+1,\ldots{},n;\xi) \quad+~ \text{(less singular)},
\end{multline}
where the sum in the numerator is over all physical polarization states of particle $\xi$, and the ``less singular'' part of the amplitude contains no poles $\frac{1}{p_\xi^2-m_\xi^2}$.
...
\begin{figure}[ht]
  ...
  \end{equation*}
  \todo{refer from the text}
  \caption{An example of factorization given by \cref{eq:factorization} due to the presence of a physical singularity. }
...
The universal factorization  property expressed by \cref{eq:factorization} is independent of perturbation theory. 
...
Unitarity can be turned into a computational method for loop amplitudes: \cite{Bern:1994cg,Bern:1994zx}
where they obtained certain (cut-constructable) class of amplitudes directly
from it's cuts by matching discontinuities of integrals.

Write the formula summing integrals with coefficients.
...
\begin{figure}[ht]
  ...
  \end{equation*}
  \caption{Illustration of a quadruple generalized cut. On the left all diagrams with the specified propagators are included.
  On the right each corner is a tree amplitude.}
  \label{fig:quad_cut}
\end{figure}
...
it to the ansatz of all possible numerator insertions instead of integral coefficients \cite{Giele:2008ve,Ellis:2007br,Ellis:2008ir,Berger:2008sj}.

Somewhat confusingly sometimes generalized unitarity methods refer to purely algebraical method of reduction widely known as OPP \cite{Ossola:2006us},
which can be applied to any numerator function.
...
The extension of the ideas to multi-loop level have been actively studied in recent years.


key concept which is behind
the main computational method of this thesis presented in \cref{chap:numunitarity}.
