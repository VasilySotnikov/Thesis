...
The scattering cross-section is an observable quantity, which
can both be predicted theoretically, and measured experimentally.
In this section we will review how cross-sections are evaluated in the framework of the QFT.

\todo{here define the cross-section with luminosity} 
...
The transition probability from the initial state to the final state $\dd{P}$ is obtained from the \emph{scattering matrix} $\mathcal{S}$ as
\todo{here connect with the cross-section} 
\begin{equation}
  \dd{P} = \frac{\Abs{\mel{f}{\mathcal{S}}{i}}}{\ip{f}{f}\ip{i}{i}} \dd{\Pi},
\end{equation}
where $\dd{\Pi}$ is the volume element of finial-state momenta. 
In a non-interacting theory $\mathcal{S}=\mathbb{I}$, so we isolate
...
The amplitudes are evaluated from the $n$-point correlation functions
...
the $n$-particle scattering amplitude $A_{i\to f}$ is obtained by taking $n$-fold residue 
of the simple poles associated to each on-shell external particle.
\todo{add formula as residues}
...
The latter can be directly derived from the Lagrangian.

\todo{Here transition to the parton model, and non-fixed number of particles in the final state.} 
